...
import { getAdminGroups } from '@/features/admin/api/adminApi';
import Header from '@/shared/components/Header';
import { colors } from '@/shared/constants/colors';

const SORT_OPTIONS = [
    { label: '이름', value: 'name' },
    { label: '멤버 수', value: 'memberCount' },
    { label: '냉장고 수', value: 'fridgeCount' },
];

export default function AdminGroupListScreen({ navigation }) {
    ...
    const fetchGroups = useCallback(async (reset = false) => {
        ...
        try {
            ...
            const data = await getAdminGroups({ search, sort, direction, page: currentPage, size: 20 });
            setGroups(reset ? data.content : prev => {
                const existingIds = new Set(prev.map(g => g.id));
                const newItems = data.content.filter(g => !existingIds.has(g.id));
    ...
    return (
        <View style={[s.root, { paddingTop: insets.top }]}>
            ...
            <View style={s.tabRow}>
                ...
                >
                    <Text style={s.tabText}>사용자</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.tab, s.tabActive]}>
                    <Text style={[s.tabText, s.tabTextActive]}>그룹</Text>
                </TouchableOpacity>
            </View>
            <TextInput
                style={s.search}
                ...
                value={search}
                onChangeText={t => { setSearch(t); setPage(0); setHasMore(true); }}
            />
            <View style={s.sortRow}>
                {SORT_OPTIONS.map(opt => (
                    <TouchableOpacity
                        ...
                        onPress={() => { setSort(opt.value); setPage(0); setHasMore(true); }}
                    >
                ...
                <TouchableOpacity
                    style={s.dirBtn}
                    onPress={() => { setDirection(d => d === 'ASC' ? 'DESC' : 'ASC'); setPage(0); setHasMore(true); }}
                >
            ...
            <FlatList
                    ...
                    >
                        <View style={{ flex: 1 }}>
                            ...
                            <View style={s.badgeRow}>
                                <Text style={s.badge}>생성자 {item.createdByName}</Text>
                ...
                onEndReached={() => fetchGroups(false)}
                onEndReachedThreshold={0.3}
                ListFooterComponent={loading ? <ActivityIndicator style={{ margin: 16 }} /> : null}
                ListEmptyComponent={!loading ? <Text style={s.empty}>그룹이 없습니다.</Text> : null}
            />
        </View>
    );
}
